...
export default function CaissierHome() {
  ...
  const loadPaiements = async () => {
    setLoading(true);
    try {
      const response = await apiRequest(`${API_ENDPOINTS.FACTURES}?limit=1000`);
  ...
  const exportFactures = () => {
    ...
    const data = factures.map(facture => ({
      'N° Facture': facture.numFact,
      'Convention': facture.numConv,
  ...
  const buildFactureHTML = (data) => {
    ...
    // Générer le numéro de facture au format AA/SNPAR
    const numFactFormatted = data.numFacture ? `${data.numFacture}/AA/SNPAR` : '....../AA/SNPAR';
...
<body>
  <div class="page">
    ...
    <div class="footer">
      ...
      <div>Web. www.fce-madagascar.net fb:FCt-madagascar</div>
    </div>
  </div>
</body>
</html>`;
  ...
  const exportConventionDetail = (convention) => {
    const data = [{
      'N° Convention': convention.numConv,
  ...
  const exportPaiementDetail = (paiement) => {
    const data = [{
      'Référence': `PAY-${String(paiement.numFact).padStart(3, '0')}`,
      'N° Facture': paiement.numFact,
                    ...
                    paiements.map((paiement, index) => (
                      ...
                      >
                        <td style={{ padding: '16px', fontSize: '14px', color: currentTheme.colors.text, fontWeight: 600 }}>
                          PAY-{String(paiement.numFact).padStart(3, '0')}
                        </td>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                  ...
                  <tbody>
                    {conventions.map((conv, index) => (
                      ...
                      >
                        <td style={{ padding: '12px 8px', fontSize: '14px', color: currentTheme.colors.text, fontWeight: 600 }}>
                          CONV-{String(conv.numConv).padStart(3, '0')}
                        </td>
                        <td style={{ padding: '12px 8px', fontSize: '14px', color: currentTheme.colors.text }}>
                    ...
                    allFactures
                      ...
                      .map(facture => (
                        <option key={facture.numFact} value={facture.numFact}>
                          Facture #{facture.numFact} - {facture.locataire?.nomcli || 'N/A'} - {facture.batiment?.montant ? formatCurrency(facture.batiment.montant) : 'N/A'} - {facture.libelles || 'Sans libellé'}
                        </option>
                      ))
                  )}
                </select>
                  ...
                  <input
                    ...
                    onInput={(e) => {
                      ...
                      if (value === '' || /^\d+$/.test(value)) {
                        setPaymentForm({ ...paymentForm, montant: value });
                      }
                    }}
                    placeholder="0"
                    step="1"
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div>
                <h2 style={{ margin: '0 0 8px', fontSize: '24px', fontWeight: 700, color: currentTheme.colors.text }}>
                  Facture #{selectedFacture.numFact}
                </h2>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'grid', gap: '12px', fontSize: '14px', color: currentTheme.colors.text, lineHeight: 1.6 }}>
                      <div>
                        ...
                        <span style={{ color: currentTheme.colors.text, fontWeight: 600 }}>#{selectedFacture.numFact}</span>
                      </div>
                      <div>
                        <strong style={{ color: currentTheme.colors.textSecondary }}>Convention :</strong>{' '}
                        <span style={{ color: currentTheme.colors.text }}>Conv. {selectedFacture.numConv}</span>
                      </div>
                      <div>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: '24px' }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'grid', gap: '12px', fontSize: '14px', color: currentTheme.colors.text, lineHeight: 1.6 }}>
                  <div>
                    <strong style={{ color: currentTheme.colors.textSecondary }}>Référence :</strong>{' '}
                    <span style={{ color: currentTheme.colors.text, fontWeight: 600 }}>
                      PAY-{String(selectedPaiement.numFact).padStart(3, '0')}
                    </span>
                  </div>
                  <div>
                    <strong style={{ color: currentTheme.colors.textSecondary }}>N° Facture :</strong>{' '}
                    <span style={{ color: currentTheme.colors.text }}>#{selectedPaiement.numFact}</span>
                  </div>
                  <div>
                    <strong style={{ color: currentTheme.colors.textSecondary }}>Convention :</strong>{' '}
                    <span style={{ color: currentTheme.colors.text }}>Conv. {selectedPaiement.numConv}</span>
                  </div>
                  <div>
